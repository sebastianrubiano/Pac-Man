...
Laberinto::Laberinto() : m_casillas{
                   ...
                   }
{
    ...
    uint Laberinto::obtenerCasilla(int x, int y) const
{
    return m_casillas[x][y];
}

bool Laberinto::casillaBloqueaEntidad(int x, int y)
{
    return m_casillas[x][y] != NINGUN_CAMPO && m_casillas[x][y] != PUNTO_PEQUENO && m_casillas[x][y] != PUNTO_GRANDE;
}

bool Laberinto::esInterseccion(int x, int y)
{
...
bool Laberinto::quitarPunto(Pacman* pacman, int &puntuacion)
{
    if (m_casillas[pacman->obtenerPosicionX()][pacman->obtenerPosicionY()] == 26)
    {
        m_casillas[pacman->obtenerPosicionX()][pacman->obtenerPosicionY()] = 30;
        pacman->comerPunto();
        puntuación += 5;
            return true;
    }
    else if (m_casillas[pacman->obtenerPosicionX()][pacman->obtenerPosicionY()] == 27)
    {
        m_casillas[pacman->obtenerPosicionX()][pacman->obtenerPosicionY()] = 30;
        puntuación += 20;
            emit seHaComidoPuntoGordo();
        return true;
    }
    return false;
}
